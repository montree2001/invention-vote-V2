...
<body>
    ...
    <div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full" data-sidebar-position="fixed" data-header-position="fixed">
        ...
        <div class="body-wrapper">
            ...
            <div class="container-fluid">
                ...
                <div class="card">
                    <div class="card-body">
                        <h1 class="card-title" style="text-align: center;font-size: 30px;">รับรองผลการลงคะแนน  <?php if($row_type['announce']==1) echo "<span class='badge bg-primary rounded-3 fw-semibold'> <i class='ti ti-circle-check'></i> รับรองผลแล้ว</span>"; ?></h1>
                        <h2 class="card-title text-center" style="font-size: 20px;"><?php echo $row_type['type_Name']; ?></h2>
                        <hr>
            ...
            WHERE invention.type_id = :type_id GROUP BY invention.invention_id ORDER BY avg_points_score DESC";
                        ...
                        <p id="text_announce" style="text-align: center;">สถานะการรับรองผล: <?php echo $row_type['announce'] == 0 ? 'ยังไม่รับรองผล' : 'รับรองผลแล้ว'; ?></p>

                        <?php if ($row_type['announce'] == 0) { ?>
                            <div class="text-center">
                                <a href="javascript:void(0);" onclick="confirmResult(<?php echo $_SESSION['type_id']; ?>);" class="btn btn-success" style="margin-top: 20px;">
                                    <i class="ti ti-check"></i> รับรองผล
                                </a>
                            </div>
                        <?php } else { ?>
                            <div class="text-center">
                                <!-- ยกเลิกการรับรองผล -->
                                <!-- <a href="javascript:void(0);" onclick="cancelResult(<?php echo $_SESSION['type_id']; ?>);" class="btn btn-danger" style="margin-top: 20px;">
                                    <i class="ti ti-x"></i> ยกเลิกการรับรองผล
                                </a> -->

                                <p style="color: red;">*หากต้องการยกเลิกการรับรองผล กรุณาติดต่อผู้ดูแลระบบ</p>
    ...
    ";
            ...
            <script>
    function confirmResult(type_id) {
        Swal.fire({
            title: 'คุณแน่ใจหรือไม่?',
            text: "คุณต้องการที่จะรับรองและประกาศผลใช่หรือไม่?",
            icon: 'question',
            showCancelButton: true,
            confirmButtonText: 'รับรอง',
            cancelButtonText: 'ยกเลิก'
        }).then((result) => {
            if (result.isConfirmed) {
                // ถ้าผู้ใช้ยืนยันการรับรอง
                // ส่งรหัสประเภทสิ่งประดิษฐ์ไปยังหน้า update_announce.php
                window.location.href = '../process/update_announce.php?type_id=' + type_id;
            }
        });
    }

    function cancelResult(type_id) {
        Swal.fire({
            title: 'คุณแน่ใจหรือไม่?',
            text: "คุณต้องการที่จะยกเลิกการรับรองผลใช่หรือไม่?",
            icon: 'question',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                // ถ้าผู้ใช้ยืนยันการยกเลิกการรับรองผล
                // ส่งรหัสประเภทสิ่งประดิษฐ์ไปยังหน้า cancel_result.php
                window.location.href = '../process/cancel_result.php?type_id=' + type_id;
            }
        });
    }
</script>
