                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        while ($row_sum_topic = $stmt_sum_topic->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            //นับคะแนนรวม
                            $sql_check_vote_sum = "SELECT SUM(score) as score_vote FROM vote WHERE invention_id = :invention_id AND committee_id = :committee_id";
                            $stmt_check_vote_sum = $pdo->prepare($sql_check_vote_sum);
                            $stmt_check_vote_sum->bindParam(':invention_id', $row['invention_id'], PDO::PARAM_INT);
                            $stmt_check_vote_sum->bindParam(':committee_id', $_SESSION['user_id'], PDO::PARAM_INT);
                            $stmt_check_vote_sum->execute();
                            $row_check_vote_sum = $stmt_check_vote_sum->fetch(PDO::FETCH_ASSOC);

                            if ($row_check_vote_sum['score_vote'] == "") {
                                $count_check_vote_sum = 0;
                            } else {
                                $count_check_vote_sum = $row_check_vote_sum['score_vote'];
                            }
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <?php } else { ?>
                                        <a style="margin-top: 20px;" href="" class="btn btn-success disabled"><i class="ti ti-ban"></i> ลงคะแนน</a>

                                    <?php } ?>

                                    <!-- คะแนนที่ได้ -->
                                    <div style="margin-top: 20px;" class="progress-bar">คะแนนที่ได้: <?php echo $count_check_vote_sum; ?> / <?php echo $sum_topic; ?> คะแนน</div>
